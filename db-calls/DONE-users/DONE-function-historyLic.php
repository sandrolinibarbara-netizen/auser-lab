...
$totalPages = "SELECT count(id) AS total FROM licenze WHERE id_utente = '$user'";
$total = $db->query($totalPages)->fetchAll(PDO::FETCH_ASSOC);
$total = isset($total[0]) ? $total[0]["total"]: 0;

$limits = " LIMIT $limit";

$query = "SELECT numero as nome, system_date_created as data_creazione, data_inizio, data_fine FROM licenze
            WHERE id_utente = '$user'".$limits;
