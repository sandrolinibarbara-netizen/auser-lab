...
$user = (int)$_SESSION[SESSIONROOT]['user'];
$group = (int)$_SESSION[SESSIONROOT]['group'];
$limit  = $length;
...
$totalPages = $group == 1
    ? "SELECT count(dirette.id) AS total FROM dirette JOIN corsi ON dirette.id_corso = corsi.id WHERE dirette.active = 1"
    : "SELECT count(dirette.id) AS total FROM dirette 
        ...
        WHERE corsi_utenti.id_utente = '$user' AND dirette.active = 1";
...
$query = $group == 1 ? "SELECT avvisi.nome as avviso, dirette.nome as nome_lezione, dirette.data_inizio as data_inizio, dirette.orario_inizio as orario_inizio, dirette.luogo as luogo, dirette.id as idDiretta, dirette.id_categoria as id_categoria, dirette.id_corso, corsi.nome as nome_corso FROM dirette 
        JOIN corsi ON dirette.id_corso = corsi.id LEFT JOIN avvisi ON dirette.id = avvisi.id_diretta WHERE dirette.active = 1".$filterCourse.$filterLoc.$dateRange.$hourRange.$limits
    : "SELECT avvisi.nome as avviso, dirette.nome as nome_lezione, dirette.data_inizio as data_inizio, dirette.orario_inizio as orario_inizio, dirette.luogo as luogo, dirette.id as idDiretta, dirette.id_categoria as id_categoria, dirette.id_corso, corsi.id as idCorso, corsi.nome as nome_corso FROM dirette 
        JOIN corsi ON dirette.id_corso = corsi.id 
        JOIN corsi_utenti ON corsi.id = corsi_utenti.id_corso
        LEFT JOIN avvisi ON dirette.id = avvisi.id_diretta
        WHERE corsi_utenti.id_utente = '$user' AND dirette.active = 1".$filterCourse.$filterLoc.$dateRange.$hourRange.$limits;
