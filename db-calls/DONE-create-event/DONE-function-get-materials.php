...
$totalPages = "SELECT count(domande.id) AS total FROM domande WHERE id_diretta IS NULL";
$total = $db->query($totalPages)->fetchAll(PDO::FETCH_ASSOC);
$total = isset($total[0]) ? $total[0]["total"]: 0;
$limits = " LIMIT $limit";

$query = "SELECT domande.id, domande.system_date_created as data, domande.titolo, categoriedomande.nome as tipoMateriale, categoriemateriali.nome as categoriaMateriale FROM domande
            ...
            WHERE id_diretta IS NULL OR domande.active = 2".$limits;
